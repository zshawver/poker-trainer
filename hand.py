...
class Hand:
    def __init__(self, table, game):
        ...
        self.pot = 0
        self.openRaiseRanges = openRaise
        # self.positions = Positions[len(self.players)]
    ...
    def newHand(self):
        ...
        self.players = self.game.players  # All players at table will play

        # self.positions.append(self.positions.pop(0)) #Shift positions
    ...
    def bet(self, player, amount):
        if player.chips < amount:
            amount = player.chips
            print("Player is all in with {}".format(player.chips))

        player.chips -= amount
        self.pot += amount

    ''' Poker Math Calculation '''
    def get_fold_frequency(self, player):
        pass

    def get_outs(self,player):
        pass

    ''' Scoring and deciding winner(s) '''
    def scoreHands(self):
        for player in self.players:
    ...
    def distributeWinnings(self):
        winnings = self.pot // len(self.winner) #Get the amount the winner[s] will be paid
        leftOver = self.pot % len(self.winner) #Get any leftover winnings not evenly distributed

        #Pay out the winner[s] the winnings
        for player in self.winner:
            player.chips += winnings

        self.pot = leftOver #Put any unpaid winnings into the pot for the next hand
